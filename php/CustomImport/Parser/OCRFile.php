...
class CustomImport_Parser_OCRFile extends CustomImport_Parser_Custom {
    ...
    protected function import_from_activity(&$record) {
      ...
      if (!$kid = kid_number_get_info($record['kid'])) {
        $message = ts(
            "Failed looking up activity for KID number '%1' at line %2", array(
          1 => $record['kid'],
          2 => $record['line_no']
            )
        );
      ...
      if ($this->test) {            
        ...
        CRM_Core_DAO::singleValueQuery("INSERT INTO civicrm_contribution_activity (contribution_id, activity_id) VALUES (%1, %2)", array(
        ...
        $this->addReportLine('ok', ts("Successfully created contribution (id: %1) for KID Number '%2' (%3) at line %4",array(
          1 => $contribution['id'],
          2 => $record['kid'],
          3 => CRM_Core_DAO::singleValueQuery("SELECT display_name FROM civicrm_contact WHERE id = %1",array(1 => array($contact_id, 'Positive'))),
          4 => $record['line_no']
        )));        
      }
    }
