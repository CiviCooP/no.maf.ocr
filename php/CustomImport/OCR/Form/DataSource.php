 ...
 *  imported.
 */
class CustomImport_OCR_Form_DataSource extends CRM_Core_Form {
    ...
    public function preProcess( ) {
        ...
        $dao->query("CREATE TEMPORARY TABLE import_job_permission_one(test int) ENGINE=InnoDB" );
        $dao->query("CREATE TEMPORARY TABLE import_job_permission_two(test int) ENGINE=InnoDB" );
        $dao->query("DROP TABLE IF EXISTS import_job_permission_one, import_job_permission_two" );
        ...
        $handler = opendir($config->uploadDir);
		    ...
		    // probably need to create some different files for this import
        ...
        if (!$this->_dataSource) {
            ...
            $this->_dataSource = CRM_Utils_Array::value(
                'dataSource',
                $_POST,
                CRM_Utils_Array::value('dataSource', $this->_params)
            );
    ...
    public function buildQuickForm( ) {
        ...
        $this->setDefaults(array('dataSource' => 'CRM_Import_DataSource_OCR'));
    ...
    private function _getDataSources() {
        ...
        //$dataSourceDIr = 'CustomImport' . DIRECTORY_SEPARATOR . 'DataSource';
        $dataSourceDir = implode(DIRECTORY_SEPARATOR, array(
            CRM_Core_Config::singleton()->extensionsDir, 'no.maf.ocr', 'php', 'CRM', 'Import', 'DataSource'
        ));
        $dataSources = array();
        ...
        while (($dataSourceFile = readdir($dataSourceHandle)) !== false) {
            $fileType = filetype($dataSourceDir . DIRECTORY_SEPARATOR . $dataSourceFile);
            $matches = array();
...
/*            $session->set('dateTypes', $dateFormats); */
	...
	//            echo 'import_table_name: '.$this->_params['import_table_name'];
			}
//echo $this->_dataSourceClassFile;
            $this->_dataSourceClassFile = str_replace( '_', '/', $this->_dataSource ) . ".php";
            require_once $this->_dataSourceClassFile;
			// echo "$this->_dataSource::postProcess( \$this->_params, \$db );";
//			print_r($this->_params);
			...
			//Do the testing here and pass your report to preview
